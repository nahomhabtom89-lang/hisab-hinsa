module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (mode === "advisor") {
      systemText = "You are an expert construction accounting advisor for a company in Juba, South Sudan and Asmara, Eritrea.\n\n" +
        "LANGUAGE: If the user writes in Tigrinya, reply in ERITREAN (Asmara) dialect. Key words: use 'hisab' not 'hisabi' for account, 'srah' for work, 'kifliti' for payment, 'genzeb' for money. Never use Tigray/Ethiopian dialect. If English, reply English.\n\n" +
      ...
      const matContext = (materials && materials.length > 0)
        ? "\nCURRENT INVENTORY:\n" + materials.map(function(m) { return "- " + m.name + ": " + m.qty + " " + m.unit + " @ $" + m.cost + "/unit"; }).join("\n")
        : "";

      systemText = "You are a construction accounting AI for a company in Juba, South Sudan. You understand Tigrinya and English.\n" +
        matContext + "\n\n" +
    ...
    let text = data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content ? data.choices[0].message.content : "";
...
function validateAndFixEntry(rawText) {
  try {
    ...
    if (amount > 0 && parsed.entries.length >= 2) {
      var totalDr = parsed.entries.reduce(function(s, e) { return s + (parseFloat(e.debit) || 0); }, 0);
      var totalCr = parsed.entries.reduce(function(s, e) { return s + (parseFloat(e.credit) || 0); }, 0);
      var isUnbalanced = Math.abs(totalDr - totalCr) > 0.01;
      var isWrongAmount = Math.abs(totalDr - amount) > 0.01 && Math.abs(totalCr - amount) > 0.01;
      if ((isUnbalanced || isWrongAmount) && parsed.entries.length === 2) {
        var drEntry = parsed.entries.find(function(e) { return (parseFloat(e.debit) || 0) > 0 || (parseFloat(e.credit) || 0) === 0; });
        var crEntry = parsed.entries.find(function(e) { return (parseFloat(e.credit) || 0) > 0 || (parseFloat(e.debit) || 0) === 0; });
        if (drEntry && crEntry) {
    ...
    parsed.entries = parsed.entries.map(function(line) {
      var acct = (line.account || "").toLowerCase();
      if (acct.includes("depreciation expense") && (parseFloat(line.credit) || 0) > 0 && (parseFloat(line.debit) || 0) === 0) {
        return Object.assign({}, line, { debit: line.credit, credit: 0, type: "expense" });
      }
      if (acct.includes("accumulated depreciation") && (parseFloat(line.debit) || 0) > 0 && (parseFloat(line.credit) || 0) === 0) {
        return Object.assign({}, line, { credit: line.debit, debit: 0, type: "contra-asset" });
      }
      return line;
    });
