...
(function () {
  ...
  window.printPOSReceiptV68 = async function (saleId) {
    ...
    nav('pos-receipt-print-v68');
  };

  // ═══════════════════════════════════════════════════════════
  // Offer to print right after Complete Sale — a persistent little
  // notice near the cart, updated with each sale's own number, rather
  // than an auto-popup (a busy cashier can ignore it and it doesn't
  // interrupt the next sale).
  // ═══════════════════════════════════════════════════════════
  ...
  function showLastReceiptNoticeV68(saleId, total, payMethod) {
    ...
    div.innerHTML = `<span>✅ Sale #${saleId} — ${fmtMoney(total)}</span><button class="btn btn-gold" style="padding:4px 10px;font-size:11px" onclick="printPOSReceiptV68(${saleId})">🖨️ Print ${docWord}</button>`;
  }
  ...
  if (typeof _origCompleteSaleV68 === 'function') {
    window.completeSale = async function () {
      ...
      if (saleEntry) showLastReceiptNoticeV68(saleEntry.id, saleEntry.amount, payMethodSnapshot);
      return result;
    };
  }

  // ── nav() — clear the printing body class when leaving the print page,
  // and make sure the notice element exists once the POS page is up ──
  ...
  if (typeof _origNavV68 === 'function') {
    window.nav = async function (page, el) {
      ...
      if (page === 'pos') injectLastReceiptNoticeV68();
      return result;
    };
  }

  console.log('✅ patch-v68.js loaded — POS sales now print a Cash Receipt or Invoice with a real receipt number, reprintable anytime via printPOSReceiptV68(saleId)');
})();
